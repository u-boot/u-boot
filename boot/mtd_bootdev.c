...
/**
 * struct mtd_bootdev_plat - per-instance bootdev platform data
 *
 * Populated lazily in get_bootflow() or eagerly by the hunt callback.
 *
 * @mtd:	whole-flash (top-level) MTD device backing this bootdev
 */
struct mtd_bootdev_plat {
	struct mtd_info *mtd;
};
...
{
	struct mtd_bootdev_plat *plat = dev_get_plat(dev);
	...
	/* Resolve the whole-flash MTD device lazily on first call */
	mtd = plat->mtd;
	if (!mtd) {
...
U_BOOT_DRIVER(mtd_bootdev) = {
	...
	.plat_auto	= sizeof(struct mtd_bootdev_plat),
};
...
static int mtd_bootdev_hunt(struct bootdev_hunter *info, bool show)
{
	...
	mtd_for_each_device(mtd) {
		...
		if (!bootdev_bind(mtd->dev, "mtd_bootdev", "mtdbootdev", &bdev)) {
			struct mtd_bootdev_plat *plat = dev_get_plat(bdev);
