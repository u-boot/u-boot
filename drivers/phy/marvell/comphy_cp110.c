...
#define BOARD_ID_MPP_ADDRESS		0xF2440150
#define AS5114_MPP_BOARD_ID		0x12
#define AS4224_POE_MPP_BOARD_ID		0x2
#define AS4224_NON_POE_MPP_BOARD_ID	0x3
#define AS4564_MPP_BOARD_ID		0x22

#define BOARD_CPLD			0x40
#define BOARD_CPLD_ADDR_LEN		1
#define BOARD_CPLD_DEV			2
#define HW_REV_OFFSET			0
#define HW_REV_LEN			1
#define HW_REV_MASK			0x0F
#define REV01 				0x8

#define SATA_TX_BASE   0xF2121844
#define SATA_TX_MASK   0xFFFFFFFF
#define SATA_TX_SETTING_VALUE 0x00004B60

#define SATA_RX_G3_REG_CTRL 0xF2121848
#define SATA_RX_G3_REG_CTRL_value   0x13D2
...
static void acc_get_board_ID(u8 *board_id)
{
	void __iomem *board_id_address = BOARD_ID_MPP_ADDRESS;
	u32 data;

	data = readl(board_id_address);
	*board_id = ((data >>19) &0x1)|(((data >> 20)&0x1) << 4 ) | (((data >>21) &0x1) <<5 ) |  (((data >>28) &0x1) <<1 ) |  (((data >>29) &0x1) <<2)| (((data >>30) &0x1) <<3 );
	return;
}

static void acc_get_hw_rev(u8 *hw_rev)
{
	int err;
	struct udevice *dev;
	u8 data = 0;

	err = i2c_get_chip_for_busnum(BOARD_CPLD_DEV, BOARD_CPLD, BOARD_CPLD_ADDR_LEN, &dev);
	if (err) {
		pr_err("%s: Cannot find CPLD I2C chip\n", __func__);
		return;
	}

	err = dm_i2c_read(dev, HW_REV_OFFSET, &data, HW_REV_LEN);
	if (err) {
		pr_err("read error from device: %p", dev);
		return;
	}
	*hw_rev = (data & HW_REV_MASK);
	return;
}
				...
				u32 type)
{
	...
	if(board_id == AS5114_MPP_BOARD_ID){
		reg_set(SATA_TX_BASE,SATA_TX_SETTING_VALUE,SATA_TX_MASK);
	}
	reg_set(SATA_RX_G3_REG_CTRL,SATA_RX_G3_REG_CTRL_value,0xffff);
				   ...
				   u32 utmi_phy_port)
{
	...
	if(board_id == AS5114_MPP_BOARD_ID)
	   data = 0x6 << UTMI_TX_CH_CTRL_AMP_OFFSET;
	else
	   if (board_id == AS4564_MPP_BOARD_ID)
	       data = 0x5 << UTMI_TX_CH_CTRL_AMP_OFFSET;
	   else
	...
	//WT20210120
	if(board_id == AS5114_MPP_BOARD_ID && hw_rev <=  REV01)
	{
		mask |= 0xFF;
		data = (data & 0xFFFFFFF0) | 0xE;
		data  = (data & 0xFFFFFFDF)| 0x20 ;
        }
        //
