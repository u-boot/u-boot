				...
				efi_uintn_t pages, uint64_t *memory)
{
	...
	case EFI_ALLOCATE_ANY_PAGES:
		/* Any page */
		addr = (u64)lmb_alloc(len, EFI_PAGE_SIZE, flags);
		if (!addr)
			return EFI_OUT_OF_RESOURCES;
		break;
	case EFI_ALLOCATE_MAX_ADDRESS:
		/* Max address */
		addr = (u64)lmb_alloc_base(len, EFI_PAGE_SIZE, *memory, flags);
		if (!addr)
			return EFI_OUT_OF_RESOURCES;
		break;
...
efi_status_t efi_free_pages(uint64_t memory, efi_uintn_t pages)
{
	...
	flags = LMB_NONOTIFY;
	len = (u64)pages << EFI_PAGE_SHIFT;
	status = lmb_free(virt_to_phys((void *)(uintptr_t)memory), len, flags);
	if (status)
		return EFI_NOT_FOUND;
