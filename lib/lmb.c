...
static long lmb_add_region(struct lmb_region *rgn, phys_addr_t base,
			   phys_size_t size, enum lmb_ops op)
{
	return lmb_add_region_flags(rgn, base, size, LMB_NONE, op);
}
...
//	lmb_dump_all_force();
//	printf("Adding memory region, base => 0x%x, size => 0x%x\n", base, size);
	start = (uintptr_t)map_sysmem(start, 0);
//	printf("YA Adding memory region, base => 0x%llx, size => 0x%x\n", start, size);
	status = efi_add_memory_map(start, (u64)size, EFI_CONVENTIONAL_MEMORY);
	if (status != EFI_SUCCESS)
		return -1;
//		printf("Error %lx adding efi mem map for addr %#x, size %#x\n",
//		       status, base, size);
	...
	for (i = lmb.memory.cnt - 1; i >= 0; i--) {
		...
		while (base && lmbbase <= base) {
			...
			if (rgn < 0) {
				...
				if (lmb_add_region_flags(&lmb.reserved,
							 base, size, flags,
							 LMB_OP_RESERVE) < 0)
					return 0;
				return base;
