			      ...
			      struct smbios_ctx *ctx)
{
	...
	obj_hdl_num = smbios_get_val_si(ctx, "contained-object-handles-number",
					SYSINFO_ID_SMBIOS_BASEBOARD_OBJS_NUM);
	if (obj_hdl_num)
		len += obj_hdl_num * SMBIOS_TYPE2_CON_OBJ_HANDLE_SIZE;
	...
	t->number_contained_objects = obj_hdl_num;
	t->manufacturer =
		smbios_add_prop_si(ctx, "manufacturer",
	...
	/* TODO: Populate the Contained Object Handles if they exist */
			      ...
			      struct smbios_ctx *ctx)
{
	...
	int elem_num, elem_len;
	size_t elem_size;

	elem_num = smbios_get_val_si(ctx, "contained-element-count",
				     SYSINFO_ID_SMBIOS_ENCLOSURE_ELEMENT_CNT);
	elem_len = smbios_get_val_si(ctx, "contained-element-record-length",
				     SYSINFO_ID_SMBIOS_ENCLOSURE_ELEMENT_LEN);
	elem_size = elem_num * elem_len;
	len += elem_size;
	...
	t->element_count = elem_num;
	t->element_record_length = elem_len;
