			 ...
			 struct tpml_pcr_selection *pcr, u32 *pcr_len)
{
	...
	*pcr_len = sizeof(pcr->count);
	...
	for (i = 0; i < pcr->count; i++) {
		...
		/* found the algorithm (bank) that matches */
		/* TODO: only select the bits we want */
		if (algo_mask & tcg2_algorithm_to_mask(sel->hash))
			pad = 0xff;

		for (j = 0; j < sel->size_of_select; j++)
			sel->pcr_select[j] = pad;

		log_debug("%s bank[%d] with algo[%#x]\n",
			  tpm2_is_active_bank(sel) ? "enable" : "disable", i,
			  sel->hash);
