		       ...
		       struct tpml_digest_values *digest_list)
{
	...
	digest_list->count = 0;
	for (i = 0; i < priv->active_pcr_count; i++) {

		switch (priv->active_pcr[i]) {
		case TPM2_ALG_SHA1:
			sha1_starts(&ctx);
...
static int tcg2_log_init(struct udevice *dev, struct tcg2_event_log *elog)
{
	...
	for (i = 0; i < count; ++i) {
		len = tpm2_algorithm_to_len(priv->active_pcr[i]);
		put_unaligned_le16(priv->active_pcr[i],
...
static int tcg2_log_parse(struct udevice *dev, struct tcg2_event_log *elog)
{
	...
	for (i = 0; i < count; ++i) {
		...
		default:
			 ...
			 */
			log_err("EventLog contains an unsupported algorithm 0x%x\n",
			        algo);
			return -1;
		}
		log_active |= mask;
