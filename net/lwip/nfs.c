...
void nfs_refresh_timeout(void)
{
	...
	new_timer = get_timer(0);
	if (new_timer - timer_start > 2000) {
		debug("%s: Timer got above nfs_timeout!!!! %lu\n", __func__, new_timer - timer_start);
	}
		    ...
		    ip_addr_t srvip)
{
	...
	} while (get_timer(timer_start) < 3000);
//	} while (get_timer(timer_start) < nfs_timeout);
